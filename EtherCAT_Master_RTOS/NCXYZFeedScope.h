...
inline bool IsNCXYZFeedNeutralFrame(const NCTranslationSnapshot& source) noexcept
{
    return IsNCTranslationSnapshotValid(source) &&
        ...
        source.rotationMode == 69 && (source.workMode == 169 || source.workMode == 168) &&
        // WORK fields 3..5 are yaw/pitch/roll, not auxiliary-axis offsets.
        // This scope admits fixed XYZ translation only; validity also proves
        ...
        source.scalingMode == 50 &&
        source.mirrorMask == 0U && source.axisIdentity.eccentricEnabled == 0U;
}
...
inline bool IsNCXYZFeedFrozenBlockAllowed(const NCBlock& block, int plane = 17) noexcept
{
    ...
    bool runtimeClear) noexcept
{
    ...
    return (frozen ? IsNCXYZFeedFrozenBlockAllowed(block, source.rotationPlane) :
